...
(defcustom buffer-stack-untracked
  '("KILL" "*Compile-Log*" "*Compile-Log-Show*"
    "*Group*" "*Completions*" "*Messages*" "*Scratch*")
...
(defun buffer-stack-show-position-buffers (buffer-stack-index buffer-stack)
  ...
  (let (up-buffer-index
    ...
    (message (concat "DOWN1:"
                     (buffer-name (nth down-buffer-index buffer-stack))
                     "  DOWN2:"
                     (buffer-name (nth (+ down-buffer-index 1) buffer-stack))
                     "  |  " "UP1:"
                     (buffer-name (nth up-buffer-index buffer-stack))
                     "  " "UP2:"
                     (buffer-name (nth (+ up-buffer-index 1) buffer-stack))
                     ))))
;;; filter stack
...
(defun buffer-stack-filter-exclusive (buffer)
  ...
  (let ((name (buffer-name buffer)))
    (not (or 
             ...
             ;added lines below for modes
             (or 
               (equal (buffer-mode buffer) 'sr-term-line-minor-mode) 
               (equal (buffer-mode buffer) 'eshell-mode) 
               (equal (buffer-mode buffer) 'dired-mode) 
               (equal (buffer-mode buffer) 'sr-mode))
 ))))
 

(defun buffer-mode (buffer-or-string)
  "Returns the major mode associated with a buffer. MTF found this on SO
  with-current-buffer changes to that buffer temporarly then back"
