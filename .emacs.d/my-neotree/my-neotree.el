...
If path is nil and no buffer file name, then use DEFAULT-PATH,"
  ...
  (let ((npath path) (fit-frame-inhibit-fitting-flag t)
        (do-open-p nil)
...
(defun neotree-enter ()
  ...
  (interactive)
  (let ((btn-full-path (neo-buffer--get-filename-current-line)) (fit-frame-inhibit-fitting-flag t))
...
(defun neotree-create-node (filename)
  ...
  (interactive
   (let* ((current-dir (neo-buffer--get-filename-current-line neo-buffer--start-node))
          (current-dir (neo-path--match-path-directory current-dir)) (fit-frame-inhibit-fitting-flag t)
...
(defun neotree-delete-node ()
  ...
  (let* ((filename (neo-buffer--get-filename-current-line))
         ...
         (deleted-p nil) (fit-frame-inhibit-fitting-flag t))
    (catch 'end
