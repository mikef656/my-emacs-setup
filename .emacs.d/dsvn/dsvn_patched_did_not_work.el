...
buffer to describe what is going on."
  ;; Clean up old output
  ;mtf
  (message "%s" svn-output-marker)
  (let ((inhibit-read-only t))
    (delete-reqgion svn-output-marker (point-max)))
...
(defun svn-unversioned-files-ok (files)
  "Add FILES after asking the user; return whether it was done."
    ...
    (with-current-buffer buf
      ...
      (save-selected-window
        ...
        (let* ((n (length files))
          ...
          (bury-buffer buf)
          (if add-them
              (svn-run 'add files "Adding files")
...
(defun svn-commit ()
  ...
  (let ((unversioned-files (svn-action-files
    ...
    (when (or (null unversioned-files)
              (svn-unversioned-files-ok unversioned-files))
