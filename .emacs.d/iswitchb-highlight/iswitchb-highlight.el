...
;
;MTF
 (remove-hook 'bookmark-bmenu-mode-hook 'bmkp-bmenu-nb-marked-in-mode-name)
            ...
            (save-excursion
              ...
              (let* ((match
                      (delq nil
                            (mapcar 
                             (lambda (x)             ;to
                               (if (and (consp x)   (stringp mode-name)
                                        (string-match (car x) mode-name))
                                    ;
                                    ;MTF-DA need to check if mode-name is a string
                                    ;bc this defun assumes it is.
                                    ;
                                    ;from:
                                    ;(and (consp x)  (stringp mode-name)
                                    ;
                                   (cdr x)))
