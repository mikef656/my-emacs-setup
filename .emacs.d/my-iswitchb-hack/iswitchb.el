;mtf c:/Users/mfitzgerald/Downloads/EMACS/emacs-24.1/lisp/iswitchb.elo
;added C-o iswitchb-select-buffer-other-window
;overwrote iswitchb-post-command to narrow display to one window
;when there is one file.
;
...
(defvar iswitchb-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "\C-v" 'iswitchb-kill-buffer);mtf
...
(defun iswitchb-define-mode-map ()
  ...
  (let (map)
    ...
    (define-key map "\C-k" 'iswitchb-kill-buffer)
    (define-key map "\C-m" 'iswitchb-exit-minibuffer)
    (define-key map "\C-o" 'iswitchb-select-buffer-other-window);added mtf
...
;mtf from the web; follow up this left wierd characters in the
;mini-buffer after doing its job.  Don't think I want it bc
;I want the choice to open the file in another window.
;which only displays the desired buffer, but in this way 
;i don't need to look at the minibuffer (in some cases). 
;; (defun iswitchb-post-command ()
;;   "Run after command in `iswitchb-buffer'."
