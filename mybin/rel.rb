...
#################################################################################
puts "----- WELCOME TO THE SURECOM RELEASE SCRIPT -----"
#################################################################################
...
################################################################################
#find newest ziped .ngc file for a netlist release,
# get it's full relative path as a pn obj
#
newest_wc_ziped_netlist_file_path=
  Pathname.glob("./trunk/**/*.zip").
    max_by {|f| File.mtime(f)}
#
  begin
    puts "Newest ziped netlist File: #{newest_wc_ziped_netlist_file_path.basename}"
    puts "Newest ziped netlist File Path: #{newest_wc_ziped_netlist_file_path}"
  rescue
...
if DO_COPY_TO_Q_STEP
  #
  unless newest_wc_bit_file_path.nil? 
    FileUtils.cp_r(newest_wc_bit_file_path, entire_path_to_q_rel_dir_pn_obj, 
      :remove_destination => COPY_TO_Q_ALLOW_OVERWRITE)
  end
  #
  unless newest_wc_ziped_netlist_file_path.nil? 
    FileUtils.cp_r(newest_wc_ziped_netlist_file_path, entire_path_to_q_rel_dir_pn_obj, 
      :remove_destination => COPY_TO_Q_ALLOW_OVERWRITE)
  end
  #
...
#
unless newest_wc_ziped_netlist_file_path.nil? 
  ziped_netlist_file_md5 = Digest::MD5.file(newest_wc_ziped_netlist_file_path).hexdigest
...
if DO_COPY_DESIGN_FILES_TO_M_ZIP_STEP
  ...
  unless newest_wc_bit_file_path.nil? 
    ...
    #
  unless newest_wc_ziped_netlist_file_path.nil? 
    FileUtils.cp_r(newest_wc_ziped_netlist_file_path, path_to_m_surecom_delivery_rel_dir_pn_obj, 
      :remove_destination => COPY_TO_M_ALLOW_OVERWRITE)
  end
    #
  unless newest_rel_rpt_file_path.nil? 
...
if CREATE_ZIP_FILE_FOR_EMAIL
  ...
  puts "\n","Windows path 2 zip: #{path_to_zip_4_email_pn_obj.to_s.gsub("\/","\\")}"
end
################################################################################
